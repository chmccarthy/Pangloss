...
# GaTech, Atlanta, GA
#
# Altered for GFF3 by Charley McCarthy, 2017.
#
# 
...
sub ReadGTF
{
  ...
  while(<IN>)
  {
# Edited if command to recognize 9-columnar GFF file and variable assignments to account for "description" in contig ID.             
    if ( /^([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\tgene_id \"(\S+)\"/ )
    {
      $seq_id = +(split(' ', $1))[0];
      $type = $3;
